...
export function SearchHub({
  ...
  const results = useMemo(() => {
    ...
    if (!q) return { programmes, resources };

    return {
      programmes: programmes.filter((programme) =>
        [programme.name, programme.faculty, programme.description]
          .filter(Boolean)
          .join(" ")
          .toLowerCase()
          .includes(q),
  ...
  return (
    <section className="container-page py-16" id="programmes">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <h2 className="section-title mt-3">Find resources by programme, owner, intake, or notes.</h2>
        <div className="mt-8">
          ...
          <input
            ...
            placeholder="Search Electrical, 2023, Google Drive, owner name..."
            value={query}
          ...
          {results.programmes.map((programme) => (
            ...
            >
              ...
              <p className="mt-3 line-clamp-3 text-sm leading-6 text-slate-600 dark:text-slate-400">
                {programme.description ?? "Open this page to view available resources."}
              </p>
            </Link>
          ))}
        </div>
